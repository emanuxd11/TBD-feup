...
df = pd.read_csv('d:\\Alex\\FEUP\\4ano\\2Semestre\\TBD\\TBD-feup\\proj2\\'+'data.csv')
...
headings = [
    ...
    (6, 'Salaries', 'E', 1000000, [0.7, 0.1, 0.2], 1),
    (7, 'Office and Operational Costs', 'E', 500000, [0.6, 0.2, 0.2], 1),
    (8, 'Capital Investments', 'E', 2000000, [0.5, 0.3, 0.2], 2),
    (9, 'Maintenance', 'E', 750000, [0.3, 0.5, 0.2], 2),
    (10, 'School Operations', 'E', 800000, [0.8, 0.1, 0.1], 3),
    (11, 'Cultural Events', 'E', 300000, [0.6, 0.2, 0.2], 3),
    (12, 'Social Assistance Programs', 'E', 600000, [0.7, 0.1, 0.2], 4),
    (13, 'Waste Collection and Treatment', 'E', 900000, [0.3, 0.5, 0.2], 5),
    # Revenues
    (18, 'IMI - Property Tax', 'R', 500000, [0.5, 0.3, 0.2], 14),
    (19, 'IMT - Property Transfer Tax', 'R', 200000, [0.5, 0.2, 0.3], 14),
    (20, 'IUC - Vehicle Tax', 'R', 150000, [0.6, 0.1, 0.3], 14),
    (21, 'State Grants', 'R', 1500000, [0.7, 0.2, 0.1], 15),
    (22, 'EU Funds', 'R', 1000000, [0.5, 0.3, 0.2], 15),
    (23, 'Licensing Fees', 'R', 100000, [0.4, 0.2, 0.4], 16),
    (24, 'Utility Service Charges', 'R', 400000, [0.5, 0.3, 0.2], 16),
...
for _, row in df.iterrows():
    ...
    for period in range(1, num_periods+1):
        ...
        for heading_id, _, h_type, base_value, weights, parent in headings:
            ...
            id = exp_id if h_type == 'E' else rev_id
            ...
            table = 'aexpenses' if h_type == 'E' else 'arevenues'

            insert_stmt = f"""INSERT INTO {table}
    SELECT {table}_t({id}, REF(h), REF(pr), REF(m), {value})
    FROM headings h, periods pr, municipalities m
    WHERE h.headingId = {heading_id} AND pr.periodId = {period} AND m.code = {muni_code};
        """
    ...
    SELECT {table}_t({id}, REF(h), REF(pr), REF(m), {value})
    FROM headings h, periods pr, municipalities m
    WHERE h.headingId = {parent} AND pr.periodId = {period} AND m.code = {muni_code};
        """
            sql_inserts.append(insert_stmt)
            if h_type == 'E':
                exp_id+=1
...
# Write SQL to file
with open('proj2\\expenses_revenues_inserts.sql', 'w') as f:
    f.write('\n'.join(sql_inserts))
f.close()
